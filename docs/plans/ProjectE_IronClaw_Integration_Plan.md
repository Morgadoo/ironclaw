...
│  AppComponents ◄── ProjectEComponents              (Phase 5)            │
│  Workspace     ◄─► WorkspaceKnowledgeStore         (Phase 6)            │
│  HeartbeatRunner ← CognitiveCycleRunner            (Phase 7)            │
...
4. `src/app.rs` — add `ProjectEComponents` field to `AppComponents`, call `init_project_e()` in `build_all()`
5. `src/main.rs` — spawn `CognitiveCycleRunner` after channel setup
...
**Goal:** Add `init_project_e()` as a 6th init phase in `AppBuilder`.

### New File: `src/project_e/init.rs`

```rust
pub struct ProjectEComponents {
    pub event_bus: Arc<EventBus>,
    pub supervisor: Arc<Mutex<Supervisor>>,
    pub module_store: Arc<ModuleStore>,
    pub deliberation_engine: Arc<DeliberationEngine>,
    pub llm_bridge: Arc<IronclawLlmBridge>,
    pub hook: Arc<CognitiveHook>,
    pub llm_health_monitor: Arc<LlmHealthMonitor>,
    pub subscriber_handle: tokio::task::JoinHandle<()>,
    pub shutdown_tx: watch::Sender<bool>,
}

pub async fn init_project_e(
...
) -> Result<ProjectEComponents, ProjectEInitError>
```
...
// AppComponents field:
#[cfg(feature = "project_e")]
pub project_e: Option<ProjectEComponents>,

// build_all() — after init_extensions():
#[cfg(feature = "project_e")]
let project_e = if self.config.project_e.enabled {
    match init_project_e(&self.config.project_e, llm.clone(), cost_guard.clone(),
                          tools.clone(), workspace.clone(), hooks.clone()).await {
        Ok(c) => { tracing::info!("Project E initialized"); Some(c) }
        Err(e) => { tracing::warn!("Project E init failed, continuing without: {e}"); None }
    }
} else { None };
```

**Resilience:** Entire init wrapped in `match`; failure → `project_e: None`; IronClaw continues normally.
...
```rust
#[cfg(feature = "project_e")]
if let Some(pe) = &app.project_e {
    let runner = CognitiveCycleRunner::new(&app.config.project_e, pe, workspace.clone());
